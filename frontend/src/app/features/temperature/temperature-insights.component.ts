      ...
      <div class="mb-5 flex gap-1 bg-gray-100 rounded-xl p-1">
        @for (r of rangeOptions; track r.value) {
          <button (click)="selectRange(r.value)"
            ...
            @if (r.value === '30d' && !tier.canSee30Days()) { <span class="ml-1 text-[10px] opacity-70">+</span> }
            @if (r.value === '90d' && !tier.canSee90Days()) { <span class="ml-1 text-[10px] opacity-70">+</span> }
          </button>
        }
      </div>
...
export class TemperatureInsightsComponent implements OnDestroy {
  ...
  // Range selector — '7d' is unrestricted, 30d/90d are Plus/Pro.
