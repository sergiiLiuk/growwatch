    ...
    <a routerLink="/forecast" class="block" *transloco="let t">
      ...
      <div class="grid grid-cols-3 gap-3">
        @for (day of cells(); track day.date) {
          <div class="rounded-2xl p-3 border-[0.5px] flex flex-col items-center text-center transition-all cursor-pointer hover:border-gray-300"
               [class]="cellClass(day.severity)">
            <span class="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">{{ day.dayLabel }}</span>
            <span class="text-2xl my-1 leading-none">{{ day.icon }}</span>
            ...
            @if (day.badges.length > 0) {
              <div class="flex gap-1 mt-1.5">
                @for (b of day.badges; track b) {
                  <span class="text-[14px] leading-none">{{ b }}</span>
                }
              </div>
            }
          </div>
  ...
  cells = computed(() => {
    ...
    return forecast.map((d, i) => {
      ...
      return {
        ...
        severity: r.severity,
        badges: r.messages.map(m => m.icon),
      };
    });
  });
